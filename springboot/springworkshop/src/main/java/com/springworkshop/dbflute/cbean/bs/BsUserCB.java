...
public class BsUserCB extends AbstractConditionBean {
     ...
     * @return this. (NotNull)
     */
    public UserCB acceptPK(Integer id) {
     ...
     *     <span style="color: #553000">cb</span>.<span style="color: #CC4747">setupSelect_MasterCategory()</span>; <span style="color: #3F7E5E">// ...().with[nested-relation]()</span>
     ...
     *     ... = <span style="color: #553000">user</span>.<span style="color: #CC4747">getMasterCategory()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
     */
    public void setupSelect_MasterCategory() {
        assertSetupSelectPurpose("masterCategory");
        if (hasSpecifiedLocalColumn()) {
            specify().columnCategoryId();
        }
        doSetupSelect(() -> query().queryMasterCategory());
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<UserCQ> {
        protected MasterCategoryCB.HpSpecification _masterCategory;
        ...
        protected void doSpecifyRequiredColumn() {
            columnId(); // PK
            if (qyCall().qy().hasConditionQueryMasterCategory()
                    || qyCall().qy().xgetReferrerQuery() instanceof MasterCategoryCQ) {
         ...
         */
        public MasterCategoryCB.HpSpecification specifyMasterCategory() {
            assertRelation("masterCategory");
            if (_masterCategory == null) {
                _masterCategory = new MasterCategoryCB.HpSpecification(_baseCB
                    , xcreateSpQyCall(() -> _qyCall.has() && _qyCall.qy().hasConditionQueryMasterCategory()
                                    , () -> _qyCall.qy().queryMasterCategory())
                    , _purpose, _dbmetaProvider, xgetSDRFnFc());
                if (xhasSyncQyCall()) { // inherits it
                    _masterCategory.xsetSyncQyCall(xcreateSpQyCall(
                        () -> xsyncQyCall().has() && xsyncQyCall().qy().hasConditionQueryMasterCategory()
                      , () -> xsyncQyCall().qy().queryMasterCategory()));
                }
            }
